...
function updateVisibleCards(containerEl, search, operation, selectedLanguages){
    ...
    Array.from(containerEl.children).forEach((cardEl) => {
        ...
        if(titleEl) {
            ...
            if(!isMatchName && operation == "AND"){ //função valida se o que está no card é igual o que está buscando
                hideCard(cardEl);
            } else if(isMatchName && operation == "OR") {
                showCard(cardEl);
                visibleCards++; //contagem de cards - resultados
            ...
            } else if (!isMatchName && operation == "OR") { //verifica se não deu match e operação for 'OR'
                let isMatchLanguage = isMatchByLanguage(cardLanguages, selectedLanguages);
                if(isMatchLanguage){
                    showCard(cardEl);
                    visibleCards++;
...
function isMatchByName(textCard, textInput) { //função para MACTCH por nome
    return textCard.toLowerCase().includes(textInput.toLowerCase());
}

function isMatchByLanguage(cardLanguages, selectedLanguages){ //função para MATCH por linguagem
